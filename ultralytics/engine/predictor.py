...
class BasePredictor:
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        self.device = None
        self.dataset = None
        self.vid_path,self.vid_writer =None,None  # dict of {save_path: video_writer, ...}
    ...
    def writer_results(self,idx,results,batch):
        p,im,_ = batch
        log_string = ''
        if len(im.shape) == 3:
            im =im[None]
        if self.source_type.wencam or self.source_type.from_img or self.source_type.tensor:
            log_string += f'{idx}:'
