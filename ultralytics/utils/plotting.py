    ...
    def box_label(self, box, label="", color=(128, 128, 128), txt_color=(255, 255, 255), rotated=False):
        ...
        if self.pil or not is_ascii(label):
            ...
            if label:
                ...
                # text_topleft_x = p1[0]
                # text_topleft_y = int(box[3] + 3)
                # overlay = self.im.copy()
                cv2.rectangle(self.im, p1, p2, color, -1, cv2.LINE_AA)  # filled
                # cv2.rectangle(overlay,(text_topleft_x,text_topleft_y),(text_topleft_x + w,text_topleft_y + h),color,-1,cv2.LINE_AA)
                # cv2.addWeighted(overlay,self.alpha,self.im,1-self.alpha,0,self.im)

                cv2.putText(
                    self.im,
                    label,
                    (p1[0], p1[1] - 2 if outside else p1[1] + h + 2),
                    # (text_topleft_x, text_topleft_y + h + 2),
                    0,
                    self.sf,
                    # self.lw / 3,
                    txt_color,
